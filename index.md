...
Type the command: `ls -c`. _What was the result?_

2. Now try this command: `mkdir NewDir` and then `ls -c`. 
_How does the new output compared to that from the previous step?_

3. As you can see, we are able to manipulate the files and directories on the server. Try `cd NewDir` and `ls -c`
